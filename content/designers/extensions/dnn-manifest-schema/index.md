...
*   **type**. A type in .NET, in a DNN library, or a third-party library. Ensures that the installation can create an object of the specified type. Example:
    ...
    > [!Tip]
    > [Fully qualify](https://msdn.microsoft.com/en-us/library/yfsftwz6(v=vs.110).aspx) a type if it is not in the App_Code folder to avoid conflicts with similarly named types from multiple sources.
